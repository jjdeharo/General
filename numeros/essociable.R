...
# Devuelve T y la serie si es sociable (periódica) y F si no lo es
# max es el número máximo de términos que se hallarán en cada serie

essociable <- function(n, max=5, salida=T) {
  num <- n
  n0 <- n
  i <- 1
  nsoc <- num
  ...
  while(T) {
    ...
    if(sum(n == num)) {
      if(num == n0 && i > 3) {
        return(list("Sociable"=T,"Serie"=nsoc[1:(length(nsoc)-1)]))
      } else {
        return(list("Sociable"=F,"Serie"=nsoc[1:(length(nsoc)-1)]))
      }
    }
    if(salida == T) cat(i,num,"\n")
    n <- c(n,num)
